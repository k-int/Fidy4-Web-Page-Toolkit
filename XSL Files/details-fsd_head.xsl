...
<!-- - - - - - - - - - -->
  ...
  <xsl:if test="'yes' = $showMap and (0 &lt; string-length(echoedData/centrePoint/@lat)) and (0 &lt; string-length(echoedData/centrePoint/@lng))">
    ...
    <script type="text/javascript">
      <xsl:if test="'print' != $print">
        ...
        addLoadEvent( function() { showItem('moreDetails6'); } );
      </xsl:if>
      ...
      Locations[0][0] = "<xsl:value-of select="concat(echoedData/centrePoint/@lat, ', ', echoedData/centrePoint/@lng)" />";
		  Locations[0][1] = "<xsl:call-template name="encode"><xsl:with-param name="uncoded" select="fsdrec:DC.Title" /></xsl:call-template>";
		  Locations[0][2] = "<xsl:apply-templates select="fsdrec:ContactDetails/fsdrec:Address" mode="fullAddress" />";
      ...
      addLoadEvent( function() { mapstraction.addMarker(makeMarker(Locations[0][0], "<xsl:value-of select="$imagesFolder" />map_1.gif", "<xsl:value-of select="$imagesFolder" />map_1.gif", "<xsl:call-template name="encode"><xsl:with-param name="uncoded" select="fsdrec:DC.Title" /></xsl:call-template>", "<b>"+Locations[0][1]+"</b><br />"+Locations[0][2])); } );
  ...
  <xsl:if test=" 3 &lt; count(fsdrec:DC.Subject)">
    <script type="text/javascript">
      addLoadEvent( function() { compressItem('categories', <xsl:value-of select="count(fsdrec:DC.Subject)" />) } );
	  </script>
  </xsl:if>
